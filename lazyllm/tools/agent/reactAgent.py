...
class ReactAgent(ModuleBase):
    def __init__(self, llm, tools: List[str], prompt: str = "", max_retries: int = 5, return_trace: bool = False):
        ...
        if not prompt:
            prompt = INSTRUCTION.replace(
                "{TOKENIZED_PROMPT}",
                WITHOUT_TOKEN_PROMPT if isinstance(llm, OnlineChatModule)
                else WITH_TOKEN_PROMPT
            )
            prompt = prompt.replace(
                "{tool_names}",
                json.dumps([t.__name__ if callable(t) else t for t in tools], ensure_ascii=False)
            )
